...
par asms (hd:tl)    = 
    ...
    CODESIZE        : os    -> L : CODESIZE   : R   : par os (hd-1:tl) 
    CODECOPY        : os    -> L : CODECOPY         : par os (3:hd+1:tl) 
    GASPRICE        : os    -> L : GASPRICE   : R   : par os (hd-1:tl)   
    EXTCODESIZE     : os    -> L : EXTCODESIZE      : par os (1:hd:tl)   
    EXTCODECOPY     : os    -> L : EXTCODECOPY      : par os (4:hd+1:tl) 
    RETURNDATASIZE  : os    -> L : RETURNDATASIZE:R : par os (hd-1:tl)   
    RETURNDATACOPY  : os    -> L : RETURNDATACOPY   : par os (3:hd+1:tl) 
    EXTCODEHASH     : os    -> L : EXTCODEHASH      : par os (1:hd:tl)   
    BLOCKHASH       : os    -> L : BLOCKHASH        : par os (1:hd:tl)   
    COINBASE        : os    -> L : COINBASE     : R : par os (hd-1:tl)   
    TIMESTAMP       : os    -> L : TIMESTAMP    : R : par os (hd-1:tl)   
    NUMBER          : os    -> L : NUMBER       : R : par os (hd-1:tl)   
    DIFFICULTY      : os    -> L : DIFFICULTY   : R : par os (hd-1:tl)   
    GASLIMIT        : os    -> L : GASLIMIT     : R : par os (hd-1:tl)   
    CHAINID         : os    -> L : CHAINID      : R : par os (hd-1:tl)   
    SELFBALANCE     : os    -> L : SELFBALANCE  : R : par os (hd-1:tl)   
    POP             : os    -> L : POP              : par os (1:hd+1:tl) 
    MLOAD           : os    -> L : MLOAD            : par os (1:hd:tl)   
    MSTORE          : os    -> L : MSTORE           : par os (2:hd+1:tl) 
    MSTORE8         : os    -> L : MSTORE8          : par os (2:hd+1:tl) 
    SLOAD           : os    -> L : SLOAD            : par os (1:hd:tl)   
    SSTORE          : os    -> L : SSTORE           : par os (2:hd+1:tl) 
    JUMP            : os    -> L : JUMP             : par os [1]         
    JUMPI           : os    -> L : JUMPI            : par os (2:hd+1:tl)  
    JUMPDEST        : os    -> L : JUMPDEST     : R : par os (hd+1:tl)    
    PC              : os    -> L : PC           : R : par os (hd-1:tl)   
    MSIZE           : os    -> L : MSIZE        : R : par os (hd-1:tl)   
    GAS             : os    -> L : GAS          : R : par os (hd-1:tl)   
    ...
    DUP1            : os    -> L : DUP1         : R : par os (hd-1:tl)   
    DUP2            : os    -> L : DUP2         : R : par os (hd-1:tl)   
    DUP3            : os    -> L : DUP3         : R : par os (hd-1:tl)   
    DUP4            : os    -> L : DUP4         : R : par os (hd-1:tl)   
    DUP5            : os    -> L : DUP5         : R : par os (hd-1:tl)   
    DUP6            : os    -> L : DUP6         : R : par os (hd-1:tl)   
    DUP7            : os    -> L : DUP7         : R : par os (hd-1:tl)   
    DUP8            : os    -> L : DUP8         : R : par os (hd-1:tl)   
    DUP9            : os    -> L : DUP9         : R : par os (hd-1:tl)   
    DUP10           : os    -> L : DUP10        : R : par os (hd-1:tl)   
    DUP11           : os    -> L : DUP11        : R : par os (hd-1:tl)   
    DUP12           : os    -> L : DUP12        : R : par os (hd-1:tl)   
    DUP13           : os    -> L : DUP13        : R : par os (hd-1:tl)   
    DUP14           : os    -> L : DUP14        : R : par os (hd-1:tl)   
    DUP15           : os    -> L : DUP15        : R : par os (hd-1:tl)   
    DUP16           : os    -> L : DUP16        : R : par os (hd-1:tl)   
    SWAP1           : os    -> L : SWAP1        : R : par os (hd+1:tl)   
    SWAP2           : os    -> L : SWAP2        : R : par os (hd+1:tl)   
    SWAP3           : os    -> L : SWAP3        : R : par os (hd+1:tl)   
    SWAP4           : os    -> L : SWAP4        : R : par os (hd+1:tl)   
    SWAP5           : os    -> L : SWAP5        : R : par os (hd+1:tl)   
    SWAP6           : os    -> L : SWAP6        : R : par os (hd+1:tl)   
    SWAP7           : os    -> L : SWAP7        : R : par os (hd+1:tl)   
    SWAP8           : os    -> L : SWAP8        : R : par os (hd+1:tl)   
    SWAP9           : os    -> L : SWAP9        : R : par os (hd+1:tl)   
    SWAP10          : os    -> L : SWAP10       : R : par os (hd+1:tl)   
    SWAP11          : os    -> L : SWAP11       : R : par os (hd+1:tl)   
    SWAP12          : os    -> L : SWAP12       : R : par os (hd+1:tl)   
    SWAP13          : os    -> L : SWAP13       : R : par os (hd+1:tl)   
    SWAP14          : os    -> L : SWAP14       : R : par os (hd+1:tl)   
    SWAP15          : os    -> L : SWAP15       : R : par os (hd+1:tl)   
    SWAP16          : os    -> L : SWAP16       : R : par os (hd+1:tl)   
    CREATE          : os    -> L : CREATE           : par os (3:hd:tl) 
    CALL            : os    -> L : CALL             : par os (7:hd:tl)
    CALLCODE        : os    -> L : CALLCODE         : par os (7:hd:tl)
    RETURN          : os    -> L : RETURN           : par os (2:hd+1:tl)
    DELEGATECALL    : os    -> L : DELEGATECALL     : par os (6:hd:tl)
    CREATE2         : os    -> L : CREATE2          : par os (4:hd:tl) 
    STATICCALL      : os    -> L : STATICCALL       : par os (6:hd:tl)
    REVERT          : os    -> L : REVERT           : par os (2:hd+1:tl)
    INVALID         : os    ->                        par os (hd:tl)  
    SELFDESTRUCT    : os    -> L : SELFDESTRUCT     : par os (1:hd+1:tl)
    INFO s          : os    ->                        par os (hd:tl)  
