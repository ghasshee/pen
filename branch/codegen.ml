...
let get_malloc ce    =                                 
    let ce    = PUSH1 (Int 64)                      >>ce    in  (*           0x40   >> .. *) 
                MLOAD                               >>ce        (*         M[0x40]  >> .. *) 
...
let compile_cnstrs cns : cnstrCode idx_list =
...
let dispatch_usualMthd idx le ce m =                                    (*                                           ABCD >> .. *)  
...
let dispatch_defaultMthd idx le ce =
...
let dispatcher le ce idx cntrct =
    ...
    let ce      =   foldl(dispatch_usualMthd idx le)ce uMthds       in  (* JUMP to Method ABCD                                   *)   
    let ce      =   POP                                     >>ce    in  (*                                                    .. *)
    let ce      =   if  default_exists tyMthds
                        then dispatch_defaultMthd idx      le ce        (* JUMP to Default Method                             .. *) 
...
let rec codegen_call le ce ly aln cr reT = 
...
and codegen_keccak256 le ce ly args =
    ...
    let ce    = mstore_mthd_args TightPack args le ce ly            in  
    let ce    = SWAP1                           >>ce                in  
                SHA3                            >>ce                  
...
and codegen_ECDSArecover le ce ly args = match args with  
    ...
    let ce    = get_malloc                        ce                in
    let ce    = mstore_mthd_args ABIPack args le  ce ly             in  
    let ce    = SWAP1                           >>ce                in  
...
and mstore_new_instance le ce ly n     =
    ...
    let ce      =   mstore_mthd_args ABIPack n.new_args le ce ly        in  (*    argssize >> alloc(wsize) >> wsize >> size >> alloc(size) >> .. *)
...
and keccak_of_aa aa le ce ly          =                                     (* S[keccak(a[i])] := the seed of array *) 
    ...
    let ce      =   idx                                 >>>>(R,le,ce,ly)in  (*                                             index >> .. *)    
    let ce      =   arr                                 >>>>(R,le,ce,ly)in  (*                                array_loc >> index >> .. *)
...
and codegen_expr_eff le ce ly aln          = function 
    ...
    | TmLog(id,args,Some ev),TyTuple[]  ->  let _,ce = codegen_log_stmt   le ce ly id args ev in PUSH1(Int 0) >> ce 
    | TmLog(id,args,None)   ,TyTuple[]  ->  err "add_stmt: type check first"
    | TmSlfDstrct expr      ,TyTuple[]  ->  let _,ce = codegen_selfDstrct le ce ly expr    in PUSH1(Int 0) >> ce 
...
and codegen_expr le ce ly aln          = function 
    ...
    | EpMinus(l,r)          ,TyUint256  ->                      op SUB l r             le ce ly             
    | EpMinus(l,r)          ,TyUint8    ->                      op SUB l r             le ce ly             
    | EpMult (l,r)          ,TyUint256  ->                      op MUL l r             le ce ly             
    | EpMult (l,r)          ,TyUint8    ->                      op MUL l r             le ce ly             
    | EpLT   (l,r)          ,TyBool     ->  assert(aln=R);      op LT  l r             le ce ly           
    | EpGT   (l,r)          ,TyBool     ->  assert(aln=R);      op GT  l r             le ce ly           
    | EpEq   (l,r)          ,TyBool     ->  assert(aln=R);      op EQ  l r             le ce ly           
    ...
    | EpCall call           ,tyRet      ->                      codegen_call le ce ly aln call tyRet
    ...
    | EpDeref(ref,tyR),ty               ->  let size    =       size_of_ty ty                           in
                                            ...
                                            let ce      =       (ref,tyR)               >>>>(R,le,ce,ly)   in  (* pushes the pointer *)
                                                                MLOAD                   >>ce 
    | e, ty                             ->  codegen_expr_eff le ce ly aln (e,ty)
...
and mstore_mthd_hash_args mthd args le ce ly =                         (*                                                        .. *)
    let ce      = mstore_mthd_hash mthd           ce            in  (*                                         &mhash >> 4 >> .. *)
    let ce      = mstore_mthd_args ABIPack args le ce ly        in  (*                              argsize >> &mhash >> 4 >> .. *)
...
and codegen_send le ce ly (s:ty _send) =
    ...
    let ce      = mstore_mthd_hash_args m args le ce ly         in  (*                               &mhash >> argssize+4 >> retbegin >> retsize >> retbegin >> retsize >> PCbkp >> .. *)
...
and codegen_assign le ce ly l r =
    let ce      = r                             >>>>(R,le,ce,ly)in
    let ce      = sstore_to_lexpr le ce ly l                    in
    le, ce

and codegen_decl le ce ly i  = 
    let pos     = stack_size ce                                 in
    let name    = i.declId                                      in
    let ce      = i.declVal                     >>>>(R,le,ce,ly)in
...
and codegen_return le ce ly ret cont =
    ...
    | (TmUnit,_)->              STOP                >>ce      
    | e         ->  let le,ce = mstore_expr le ce ly ABIPack e in
                                RETURN              >>ce    in 
    le,ce
...
and codegen_stmt ly  (le,ce)       = function 
    | TmReturn(ret,cont)            ->  codegen_return      le ce ly ret cont
    | SmAssign(l,r)                 ->  codegen_assign      le ce ly l r        
    | SmDecl i                      ->  codegen_decl        le ce ly i
    | SmExpr expr                   ->  codegen_expr_stmt   le ce ly expr
    | SmIfThen(cond,e)              ->  codegen_if_then     le ce ly cond e 
...
let codegen_mthd ly idx (le,ce) m  =
    ...
    let le      = add_mthdArgLocs    m             le           in
    let le,ce   = codegen_stmts m.mthd_body ly le ce           in
    le,ce
...
let rntimeInfo_of_rntimeCode rc ccs : rntimeInfo =
    ...
    ; rntimeCnstrSizes      = to_idx_list ccs_sizes
    ; rntimeCnOffsts        = rc.rntime_cn_offsets
    ; rntimeCnstrOffsts     = to_idx_list ccs_offsets }
