...
--     |           +-  ARG 1
--     +-  PUSH1 "00"
-- arg :: [RBTree OPCODE] -> [RBTree OPCODE] 
-- arg ts = stk ++ ts' where 
--     (ts', stk, _) = loop ts [] 1 where  
--         loop [] stk i                   = ([], stk, i) 
--         loop (RED o os:    ts) stk i    = (RED o (stk'++os') : ts', stk'', i'') where 
--                                                 (os', stk', i')     = loop os [] i
--                                                 (ts', stk'', i'')   = loop ts stk i'
--         loop (BLK o os:    ts) stk i    = (BLK o (stk'++os') : ts', stk'', i'') where
--                                                 (os', stk', i')     = loop os [] i 
--                                                 (ts', stk'', i'')   = loop ts stk i'
--     
