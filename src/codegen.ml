...
let codegen_cnstrctr_bytecode cns idx = (* return ce which contains the program *) 
    ...
    let ce      = mstore_rntime_code      ce idx           in (*                                      alloc(codesize) << codesize << i <<  ..    *)
                  RETURN                >>ce                  (*                                                                     i <<  ..    *)
...
and add_log_stmt le ce layout name args evnt =
...
let codegen_append_cntrct_bytecode le ce layout((idx,cntrct):idx*ty cntrct) =
    ...
    register_entry (Cntrct idx) label ;                   (* update the entry database with (id,pc) pair *) 
