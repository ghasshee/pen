...
let type_check (expr,(_,t)) = assert (expr = t)
...
let typecheck_multiple (exprs:ty list) (actual:ty expr list) =
    L.for_all2 (fun e (_,a)-> e=a) exprs actual




let rec assignTy_call tyCns cname tyenv (fncall:unit fncall) : ty fncall * ty  =
...
and assignTy_msg tyCns cname tyenv (msg:unit msg) : ty msg =
...
and assignTy_new_expr tyCns cname tenv e : ty new_expr*string (* name of the cntrct just created *) =
...
and assignTy_return tyCns cname tyenv (ret:unit return) : ty return =
...
and assignTy_varDecl tyCns cname tyenv (vd:unit varDecl): ty varDecl * tyEnv =
...
and assignTy_stmt tyCns cname tyenv (src:unit stmt) : (ty stmt * tyEnv) =
    match src with
...
and assignTy_stmts tyCns cname tyenv = function (* unit stmt list -> ty stmt list*) 
    | []            ->  []
...
let assignTy_toplevel tyCns (evs:evnt idx_list) (top:unit toplevel) : ty toplevel 
    = match top with
...
let assignTys (tops : unit toplevel idx_list) : ty toplevel idx_list =
    ...
    let evs : evnt idx_list    = filter_map (function   | Event e  -> Some e
                                                        | _        -> None   ) tops in
    map (assignTy_toplevel tys evs) tops
