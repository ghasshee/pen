...
contract:
    | CONTRACT IDENT; plist(arg); LBRACE; list(case); RBRACE;
                                { Syntax.Contract(  { Syntax.contract_cases     = $5
                                                    ; contract_name             = $2
                                                    ; contract_arguments        = $3  }   ) }
    | EVENT IDENT plist(event_arg) SEMICOLON;
                                { Syntax.Event      { Syntax.event_arguments    = $3
                                                    ; event_name                = $2  }   };

case:
    | case_header block         {   { Syntax.case_header    = $1
                                    ; Syntax.case_body      = $2    }    }
    ;

block:
    | LBRACE; scs = list(sentence);RBRACE { scs }
    ;

case_header:
    | DEFAULT                             { Syntax.DefaultCaseHeader }
    | CASE; LPAR; return_typ = typ; name = IDENT; args = plist(arg); RPAR 
    { Syntax.UsualCaseHeader { case_return_typ = [return_typ] (* multi returns not supported *); Syntax.case_name = name; case_arguments = args } }
    | CASE LPAR VOID; name = IDENT; args = plist(arg); RPAR 
    { Syntax.UsualCaseHeader { case_return_typ = []; Syntax.case_name = name; case_arguments = args } };

arg:
    | t = typ; i = IDENT                {   { Syntax.arg_typ        = t
                                            ; Syntax.arg_ident      = i
...
event_arg:
  ...
  | t = typ;INDEXED;i = IDENT           { { Syntax.event_arg_body =
        { Syntax.arg_typ = t
        ; Syntax.arg_ident = i
        ; Syntax.arg_location = None
        }
...
typ:
  ...
  | key = typ; RARROW; value = typ;     { Syntax.MappingType (key, value) }
