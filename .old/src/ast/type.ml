...
let rec is_known_type ( contract_interfaces : Contract.contract_interface Assoc.contract_id_assoc) = Syntax.(function 
    | Uint256Type                   -> true
    | Uint8Type                     -> true
    | Bytes32Type                   -> true
    | AddressType                   -> true
    | BoolType                      -> true
    | ReferenceType l               ->  BatList.for_all (is_known_type contract_interfaces) l
    | TupleType l                   ->  BatList.for_all (is_known_type contract_interfaces) l
    | MappingType (a, b)            ->  is_known_type contract_interfaces a && is_known_type contract_interfaces b
    | ContractArchType contract     ->  is_known_contract contract_interfaces contract
    | ContractInstanceType contract ->  is_known_contract contract_interfaces contract
    | VoidType                      -> true
  )
...
let call_arg_expectations (contract_interfaces : Contract.contract_interface Assoc.contract_id_assoc) mtd : typ list -> bool =
  match mtd with
  | "pre_ecdsarecover"  ->  (fun x -> x = [Bytes32Type; Uint8Type; Bytes32Type; Bytes32Type])
  | "keccak256"         ->  (fun _ -> true)
  | "iszero"            ->  (fun x -> x = [Bytes32Type]||x=[Uint8Type]||x=[Uint256Type]||x=[BoolType]||x=[AddressType])
  | name                ->
...
let check_args_match (contract_interfaces : Contract.contract_interface Assoc.contract_id_assoc) (args : (typ * 'x) exp list) (call_head : string option) =
    let expectations : typ list -> bool =   match call_head with
