...
let interpret_interface_type    = Syntax.(function 
    ...
    | TupleType _                   -> failwith "interpret_interface_type: tupleType not supported yet"
    | MappingType (_, _)            -> failwith "interpret_interface_type: mappingType not supported"
...
let to_typ                  = Syntax.(function  
    | InterfaceUint x               -> if(x<0||x>256)then failwith "integer: out of range"; Uint256Type
    | InterfaceBytes x when x=32    -> Bytes32Type
    | InterfaceBool                 -> BoolType
    | InterfaceAddress              -> AddressType
    )

let interface_typ_size      = function 
    | InterfaceUint _                 -> 32 (* bytes *) 
    | InterfaceAddress                -> 32 
    | InterfaceBool                   -> 32
    | InterfaceBytes _                -> 32
...
let get_interface_typ (raw : Syntax.arg) : (string * interface_typ) option = match Syntax.(raw.arg_typ) with
...
let rec argument_sizes_to_positions_inner ret used = function 
  | []          -> List.rev ret
...
let print_arg_loc r =
  List.iter (fun (name,loc) -> Printf.printf "argument %s at %s\n" name (Location.as_string loc)) r
...
let strip_0x h          =   if BatString.starts_with h "0x" then BatString.tail h 2 else h

let add_hex sha3_256 h  =
  let h                     =   strip_0x h in
  let add_byte c            =   sha3_256#add_char c in
  let chars                 =   BatString.explode h in
  let rec work chars        =   match chars with
    | []                        -> ()
...
let string_of_interface_type    = function 
  | InterfaceUint x                 ->  "uint"^(string_of_int x)
  | InterfaceBytes x                ->  "bytes"^(string_of_int x)
