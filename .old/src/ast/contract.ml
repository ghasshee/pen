...
let case_interface_of raw       = match Syntax.(raw.case_header) with
  | Syntax.UsualCaseHeader header   ->  { Ethereum.sig_return =
  ...
  | Syntax.DefaultCaseHeader        ->  { Ethereum.sig_return = []
     ; sig_name = "" (* is this a good choice? *)
     ; sig_args = []    }

type contract_interface =
  { contract_interface_name : string            (* the name of the contract. *)
...
let rec collect_continuation_in_sentence = Syntax.(function 
    | AbortSentence                 ->  []
    | ReturnSentence r              ->  begin
    ...
    | IfThenOnly (_, ss)            ->  collect_continuation_in_sentences ss
    | IfThenElse (_, s, t)          -> (collect_continuation_in_sentences)s @(collect_continuation_in_sentences t)
    | ExpSentence _                 ->  []
    | LogSentence _                 ->  []  
    )
and collect_continuation_in_sentences ss    =  List.concat (List.map collect_continuation_in_sentence ss)
...
let find_method_sig_in_contract (method_name : string) (i : contract_interface) : case_interface option =
    Misc.first_some 
        (fun case_inter   -> if case_inter.Ethereum.sig_name = method_name then Some case_inter else None) 
