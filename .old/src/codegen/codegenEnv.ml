...
let code_length ce                  =   Evm.size_of_program ce.ce_program

let stack_size ce                   =   ce.ce_stack_size

let set_stack_size ce i             =   { ce with ce_stack_size = i }

let append_instruction  orig i      =
    if orig.ce_stack_size < Evm.stack_eaten i then failwith "stack underflow"
    else    (match i with
                | Evm.JUMPDEST l ->   begin
                    try ignore (Label.lookup_value l)
                    with Not_found ->
                        Label.register_value l (code_length orig) end
                | _ -> ()
            ); 
            ...
            else    { ce_stack_size = new_stack_size
...
let contract_lookup ce cid          =   try Assoc.choose_contract cid ce.ce_contracts
                                        with e ->
