...
module LTL2Buchi {-# WARNING "LTLtoBuchi to be used" #-}  where
...
consistentBool cl a =
  ...
  all noContradiction (toList a)
  where
    noContradiction f = case f of
      Not (AP s) -> not (AP s ∈ a)
      AP s       -> not (Not (AP s) ∈ a)
      And p q    -> (And p q ∈ a) == (p ∈ a && q ∈ a)
      Or  p q    -> (Or  p q ∈ a) == (p ∈ a || q ∈ a)
      _          -> True
...
allStates cl = filter (consistentBool cl &&$ respectsAndOr cl) (subsets cl) 

-- ensure And/Or expansion consistency for all And/Or in closure
respectsAndOr :: Set LTL -> Set LTL -> Bool
respectsAndOr cl a = all check (toList cl)
  where
    check (And p q) = (And p q ∈ a)  == (p ∈ a && q ∈ a)
    check (Or  p q) = (Or  p q ∈ a)  == (p ∈ a || q ∈ a)
    check _         = True
...
acceptSetForUntil sts (U _ psi) = fromList [ s | s <- sts, psi ∈  s ]
acceptSetForUntil _ _           = φ
...
gnbaToNba (sts, inits, trans, accSets) =
  let k = length accSets
      ...
      transN    = do
        ((s,t)) <- trans
        i       <- [0..k-1]
        let j   = if t ∈  accSets !! i then (i+1) `mod` k else i
        return ((s,i),(t,j))
      accN      = fromList [ (s,i) | s <- sts, i <- [0..k-1], s ∈ accSets !! i ]
...
showState s = "{" ++ intercalate "," (map showLTL (toList s)) ++ "}"
...
printGNBA (sts, inits, trans, accs) = do
  ...
  mapM_ (\s -> putStrLn $ "-- set: " ++ intercalate "," (map showState (toList s))) accs

printNBA :: (Show state, Ord state) => NBA state -> IO ()
printNBA (sts, inits, trans, acc) = do
  ...
  mapM_ (putStrLn . show) (toList acc)
